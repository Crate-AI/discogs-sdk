...
export function createMockOAuthResponse(token: string, secret: string): MockOAuthResponse {
    return {
        ok: true,
        status: 200,
        data: {
...
export function setupMockResponses(mockHttpClient: MockHttpClient) {
    ...
    mockHttpClient.setMockResponse('oauth/request_token', 
        createMockOAuthResponse('test_token', 'test_secret')
    );

    // Set up access token response
    mockHttpClient.setMockResponse('oauth/access_token',
        createMockOAuthResponse('test_access_token', 'test_access_secret')
    );
}

// Helper function to verify storage state
export async function verifyStorageState(storage: MockStorageAdapter, expected: Record<string, string>) {
    for (const [key, value] of Object.entries(expected)) {
        const storedValue = await storage.getItem(key);
        if (storedValue !== value) {
            throw new Error(`Storage mismatch for ${key}: expected ${value}, got ${storedValue}`);
        }
    }
}